...
#
define btelasticsearch::service::systemd(
  ...
  $notify_service = $btelasticsearch::restart_config_change ? {
    true  => [ Exec["systemd_reload_${name}"], Service["btelasticsearch-instance-${name}"] ],
  ...
  if ( $ensure == 'present' ) {
    ...
    if ($init_defaults_file != undef) {
      file { "${btelasticsearch::params::defaults_location}/elasticsearch-${name}":
        ensure => $ensure,
        source => $init_defaults_file,
        owner  => 'root',
        group  => '0',
        mode   => '0644',
        before => Service["btelasticsearch-instance-${name}"],
      ...
      if ($init_defaults != undef and is_hash($init_defaults) ) {
        ...
        if(has_key($init_defaults, 'ES_USER')) {
          if($init_defaults['ES_USER'] != $btelasticsearch::elasticsearch_user) {
      ...
      $init_defaults_pre_hash = {
        'ES_USER' => $btelasticsearch::btelasticsearch_user,
        'ES_GROUP' => $btelasticsearch::btelasticsearch_group,
        'MAX_OPEN_FILES' => '65536',
      }
      $new_init_defaults = merge($init_defaults_pre_hash, $init_defaults)

      augeas { "defaults_${name}":
        incl    => "${btelasticsearch::params::defaults_location}/elasticsearch-${name}",
        lens    => 'Shellvars.lns',
    ...
    if ($init_template != undef) {
      ...
      btelasticsearch_service_file { "${btelasticsearch::params::systemd_service_path}/elasticsearch-${name}.service":
        ensure            => $ensure,
        content           => file($init_template),
        defaults_location => $btelasticsearch::defaults_location,
        group             => $btelasticsearch::btelasticsearch_group,
        instance          => $name,
        memlock           => $memlock,
        nofile            => $nofile,
        package_name      => $btelasticsearch::package_name,
        pid_dir           => $btelasticsearch::pid_dir,
        user              => $btelasticsearch::btelasticsearch_user,
        notify            => $notify_service,
      } ->
      file { "${btelasticsearch::params::systemd_service_path}/elasticsearch-${name}.service":
        ensure => $ensure,
        before => Service["btelasticsearch-instance-${name}"],
        notify => $notify_service,
      }
  ...
  $service_require = Exec["systemd_reload_${name}"]
    ...
    file { "${btelasticsearch::params::systemd_service_path}/elasticsearch-${name}.service":
      ...
      subscribe => Service["btelasticsearch-instance-${name}"],
  ...
  # action
  service { "btelasticsearch-instance-${name}":
    ensure     => $service_ensure,
    enable     => $service_enable,
    name       => "elasticsearch-${name}.service",
    hasstatus  => $btelasticsearch::params::service_hasstatus,
    hasrestart => $btelasticsearch::params::service_hasrestart,
    pattern    => $btelasticsearch::params::service_pattern,
    provider   => 'systemd',
