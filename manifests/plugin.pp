...
define btelasticsearch::plugin(
  ...
  case $ensure {
    ...
    'absent': {
      $_file_ensure = $ensure
      $_file_before = File[$btelasticsearch::plugindir]
    }
    default: {
  ...
  if ! empty($instances) and $btelasticsearch::restart_plugin_change {
    Btelasticsearch_plugin[$name] {
      notify +> Btelasticsearch::Instance[$instances],
    }
  }
  ...
  } elsif ($btelasticsearch::proxy_url != undef) {
    $_proxy = $btelasticsearch::proxy_url
  } else {
    $_proxy = undef
  }
  ...
  if ($source != undef) {
    ...
    $file_source = "${btelasticsearch::package_dir}/${basefilename}"

    file { $file_source:
      ensure => 'file',
      source => $source,
      before => Btelasticsearch_plugin[$name],
    }
  ...
  btelasticsearch_plugin { $name:
    ...
    url         => $url,
    proxy       => $_proxy,
    plugin_dir  => $::btelasticsearch::plugindir,
    plugin_path => $module_dir,
  } ->
  file { "${btelasticsearch::plugindir}/${_module_dir}":
    ensure  => $_file_ensure,
    mode    => 'o+Xr',
    recurse => true,
    before  => $_file_before,
