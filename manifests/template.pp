...
#
define btelasticsearch::template (
  $ensure                  = 'present',
  $file                    = undef,
  $source                  = undef,
  $content                 = undef,
  $api_protocol            = $btelasticsearch::_api_protocol,
  $api_host                = $btelasticsearch::api_host,
  $api_port                = $btelasticsearch::api_port,
  $api_timeout             = $btelasticsearch::api_timeout,
  $api_basic_auth_username = $btelasticsearch::_api_basic_auth_username,
  $api_basic_auth_password = $btelasticsearch::_api_basic_auth_password,
  $api_ca_file             = $btelasticsearch::api_ca_file,
  $api_ca_path             = $btelasticsearch::api_ca_path,
  $validate_tls            = $btelasticsearch::_validate_tls,
) {
  validate_string(
    $api_protocol,
    $api_host,
  ...
  require btelasticsearch

  es_instance_conn_validator { "${name}-template":
    server => $api_host,
    port   => $api_port,
  } ->
  btelasticsearch_template { $name:
    ensure       => $ensure,
    content      => $_content,
    source       => $_source,
