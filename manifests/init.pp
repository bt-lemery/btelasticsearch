...
#
class btelasticsearch(
  $ensure                         = $btelasticsearch::params::ensure,
  $status                         = $btelasticsearch::params::status,
  $restart_on_change              = $btelasticsearch::params::restart_on_change,
  $restart_config_change          = $btelasticsearch::restart_on_change,
  $restart_package_change         = $btelasticsearch::restart_on_change,
  $restart_plugin_change          = $btelasticsearch::restart_on_change,
  $autoupgrade                    = $btelasticsearch::params::autoupgrade,
  $version                        = false,
  $package_provider               = 'package',
  $package_url                    = undef,
  $package_dir                    = $btelasticsearch::params::package_dir,
  $package_name                   = $btelasticsearch::params::package,
  $package_pin                    = true,
  $purge_package_dir              = $btelasticsearch::params::purge_package_dir,
  $package_dl_timeout             = $btelasticsearch::params::package_dl_timeout,
  $proxy_url                      = undef,
  $btelasticsearch_user             = $btelasticsearch::params::btelasticsearch_user,
  $btelasticsearch_group            = $btelasticsearch::params::btelasticsearch_group,
  $configdir                      = $btelasticsearch::params::configdir,
  $purge_configdir                = $btelasticsearch::params::purge_configdir,
  $service_provider               = 'init',
  $init_defaults                  = undef,
  $init_defaults_file             = undef,
  $init_template                  = "${module_name}/etc/init.d/${btelasticsearch::params::init_template}",
  $config                         = undef,
  $config_hiera_merge             = false,
  $datadir                        = $btelasticsearch::params::datadir,
  $logdir                         = $btelasticsearch::params::logdir,
  $plugindir                      = $btelasticsearch::params::plugindir,
  $java_install                   = false,
  $java_package                   = undef,
  $manage_repo                    = false,
  $repo_version                   = undef,
  ...
  $default_logging_level          = $btelasticsearch::params::default_logging_level,
  $repo_stage                     = false,
  $instances                      = undef,
  ...
  $validate_tls                   = true,
  $system_key                     = undef,
  $file_rolling_type              = $btelasticsearch::params::file_rolling_type,
  $daily_rolling_date_pattern     = $btelasticsearch::params::daily_rolling_date_pattern,
  $rolling_file_max_backup_index  = $btelasticsearch::params::rolling_file_max_backup_index,
  $rolling_file_max_file_size     = $btelasticsearch::params::rolling_file_max_file_size,
) inherits btelasticsearch::params {

  anchor {'btelasticsearch::begin': }
  ...
  if is_array($btelasticsearch::params::service_providers) {
    # Verify the service provider given is in the array
    if ! ($service_provider in $btelasticsearch::params::service_providers) {
    ...
    $real_service_provider = $btelasticsearch::params::service_providers
  }
  ...
  if $config_hiera_merge == true {
    $x_config = hiera_hash('btelasticsearch::config', $config)
  } else {
    $x_config = $config
  }
  ...
  if $instances_hiera_merge == true {
    $x_instances = hiera_hash('btelasticsearch::instances', $::btelasticsearch::instances)
  } else {
  ...
  if $x_instances {
    ...
    create_resources('btelasticsearch::instance', $x_instances)
  }
  ...
  if $plugins_hiera_merge == true {
    $x_plugins = hiera_hash('btelasticsearch::plugins', $::btelasticsearch::plugins)
  } else {
    $x_plugins = $plugins
  }
  ...
  if $x_plugins {
    ...
    create_resources('btelasticsearch::plugin', $x_plugins)
  }
  ...
  if $package_pin {
    class { 'btelasticsearch::package::pin':
      before => Class['btelasticsearch::package'],
    }
  }
  ...
  if ($manage_repo == true) {
    ...
    if defined(Class['btelasticsearch::package::pin']) {
      Class['btelasticsearch::package::pin']
      -> Class['btelasticsearch::repo']
    }
  ...
  if $ensure == 'present' {
    ...
    Class['btelasticsearch::config']
    -> Btelasticsearch::Plugin <| ensure == 'present' or ensure == 'installed' |>
    Btelasticsearch::Plugin <| ensure == 'absent' |>
    -> Class['btelasticsearch::config']
    Class['btelasticsearch::config']
    -> Btelasticsearch::Instance <| |>
    Class['btelasticsearch::config']
    -> Btelasticsearch::Shield::User <| |>
    Class['btelasticsearch::config']
    -> Btelasticsearch::Shield::Role <| |>
    Class['btelasticsearch::config']
    -> Btelasticsearch::Template <| |>
  ...
  Btelasticsearch::Shield::Role <| |>
  -> Btelasticsearch::Shield::User <| ensure == 'present' |>
  # Ensure users are removed before referenced roles are managed
  Btelasticsearch::Shield::User <| ensure == 'absent' |>
  -> Btelasticsearch::Shield::Role <| |>
  ...
  Btelasticsearch::Instance <| ensure == 'present' |>
  -> Btelasticsearch::Template <| |>
  # Ensure instances are stopped after managing templates
  Btelasticsearch::Template <| |>
  -> Btelasticsearch::Instance <| ensure == 'absent' |>
}
