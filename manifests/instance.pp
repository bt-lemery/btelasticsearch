...
#
define btelasticsearch::instance(
  $ensure                        = $btelasticsearch::ensure,
  $status                        = $btelasticsearch::status,
  $config                        = undef,
  $configdir                     = undef,
  $datadir                       = undef,
  $logdir                        = undef,
  ...
  $logging_level                 = $btelasticsearch::default_logging_level,
  $service_flags                 = undef,
  $init_defaults                 = undef,
  $init_defaults_file            = undef,
  $init_template                 = $btelasticsearch::init_template,
  $ssl                           = false,
  ...
  $keystore_path                 = undef,
  $system_key                    = $btelasticsearch::system_key,
  $file_rolling_type             = $btelasticsearch::file_rolling_type,
  $daily_rolling_date_pattern    = $btelasticsearch::daily_rolling_date_pattern,
  $rolling_file_max_backup_index = $btelasticsearch::rolling_file_max_backup_index,
  $rolling_file_max_file_size    = $btelasticsearch::rolling_file_max_file_size,
) {

  require btelasticsearch::params
  ...
  $notify_service = $btelasticsearch::restart_config_change ? {
    true  => Btelasticsearch::Service[$name],
    false => undef,
  }

  # Instance config directory
  if ($configdir == undef) {
    $instance_configdir = "${btelasticsearch::configdir}/${name}"
  } else {
  ...
  if ($ensure == 'present') {
    ...
    if ($datadir == undef) {
      if (is_array($btelasticsearch::datadir)) {
        $instance_datadir = array_suffix($btelasticsearch::datadir, "/${name}")
      } else {
        $instance_datadir = "${btelasticsearch::datadir}/${name}"
      }
    } else {
    ...
    if ($logging_file != undef) {
    ...
    } elsif ($btelasticsearch::logging_file != undef) {
      $logging_source = $btelasticsearch::logging_file
      $logging_content = undef
      $_log4j_content = undef
    } else {

      if(is_hash($btelasticsearch::logging_config)) {
        $main_logging_config = deep_implode($btelasticsearch::logging_config)
      } else {
        $main_logging_config = { }
      }
      ...
      $logging_hash = merge(
        $btelasticsearch::params::logging_defaults,
        $main_logging_config,
        $instance_logging_config
      )
      ...
      } elsif ($btelasticsearch::logging_template != undef) {
        $logging_content = template($btelasticsearch::logging_template)
        $_log4j_content = template($btelasticsearch::logging_template)
      } else {
    ...
    if ($btelasticsearch::x_config != undef) {
      $main_config = deep_implode($btelasticsearch::x_config)
    } else {
      $main_config = { }
    }
    ...
    if ($logdir == undef) {
      $instance_logdir = "${btelasticsearch::logdir}/${name}"
    } else {
    ...
    exec { "mkdir_logdir_elasticsearch_${name}":
      ...
      require => Class['btelasticsearch::package'],
    ...
    file { $instance_logdir:
      ...
      mode    => '0644',
      require => Class['btelasticsearch::package'],
      before  => Btelasticsearch::Service[$name],
    }
    ...
    file { $instance_datadir:
      ...
      group   => undef,
      mode    => '0644',
      require => [ Exec["mkdir_datadir_elasticsearch_${name}"], Class['btelasticsearch::package'] ],
      before  => Btelasticsearch::Service[$name],
    }

    exec { "mkdir_configdir_elasticsearch_${name}":
      command => "mkdir -p ${instance_configdir}",
      creates => $btelasticsearch::configdir,
      require => Class['btelasticsearch::package'],
      before  => Btelasticsearch::Service[$name],
    }
    ...
    file { $instance_configdir:
      ...
      mode    => '0644',
      purge   => $btelasticsearch::purge_configdir,
      force   => $btelasticsearch::purge_configdir,
      require => [ Exec["mkdir_configdir_elasticsearch_${name}"], Class['btelasticsearch::package'] ],
      before  => Btelasticsearch::Service[$name],
    }
    ...
    file { "${instance_configdir}/scripts":
      ...
      target => "${btelasticsearch::params::homedir}/scripts",
    }

    file { "${instance_configdir}/shield":
      ensure  => 'directory',
      mode    => '0644',
      source  => "${btelasticsearch::params::homedir}/shield",
      recurse => 'remote',
      owner   => 'root',
      group   => '0',
      before  => Btelasticsearch::Service[$name],
    ...
    if (is_hash($btelasticsearch::init_defaults)) {
      $global_init_defaults = $btelasticsearch::init_defaults
    } else {
    ...
    $require_service = Class['btelasticsearch::package']
